...
if __name__ == "__main__":
    ...
    # 基础训练参数
    parser.add_argument("--out_dir", type=str, default="pretrain_model_200M", help="模型输出目录")
    parser.add_argument("--epochs", type=int, default=1, help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=16, help="批次大小")
    parser.add_argument("--learning_rate", type=float, default=2e-4, help="学习率")
    parser.add_argument("--device", type=str, default="cuda:3" if torch.cuda.is_available() else "cpu", help="训练设备")
    parser.add_argument("--dtype", type=str, default="bfloat16", help="数据类型")
    
    # 实验跟踪和数据加载参数
    parser.add_argument("--use_swanlab", action="store_true", help="是否使用SwanLab进行实验跟踪")
    parser.add_argument("--num_workers", type=int, default=8, help="数据加载的工作进程数")
    parser.add_argument("--data_path", type=str, default="../train_data/seq_monkey_datawhale.jsonl", help="训练数据路径")
    ...
    # 多GPU训练参数
    parser.add_argument("--gpus", type=str, default='3', help="使用的GPU ID，用逗号分隔 (例如: '0,1,2')")
    ...
    if args.use_swanlab:
        ...
        run = swanlab.init(
            project="SLM-pretrain",  # 项目名称
            experiment_name="SLM",  # 实验名称
            config=args,  # 保存所有超参数
        )
